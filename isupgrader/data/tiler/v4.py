...
def _pad_image(image: np.ndarray, pad_len: int, pad_val: str) -> Tuple[None, np.ndarray]:
...
def _transpose_image(image: np.ndarray) -> Tuple[None, np.ndarray]:
...
def compute_coords(image: np.ndarray,
                   patch_size: int = 256,
                   precompute: bool = False,
                   min_patch_info: float = 0.35,
                   min_axis_info: float = 0.35,
                   min_consec_axis_info: float = 0.35,
                   min_decimal_keep: float = 0.7) -> Union[Tuple[np.ndarray, np.ndarray], np.ndarray]:
    """
    Input:
...
def generate_tiles(image: np.ndarray,
                   ...
                   min_axis_info: float = 0.35,
                   min_consec_axis_info: float = 0.35,
                   min_decimal_keep: float = 0.7,
                   only_tiles: bool = True,) -> Union[Tuple[np.ndarray, np.ndarray], np.ndarray]:
    """Generate tiles from an image

    Args:
        image (np.ndarray): Image to tile
        tile_size (int): size of tiles
        min_patch_info (float, optional): [description]. Defaults to 0.35.
        min_axis_info (float, optional): [description]. Defaults to 0.35.
        min_consec_axis_info (float, optional): [description]. Defaults to 0.35.
        min_decimal_keep (float, optional): [description]. Defaults to 0.7.
        only_tiles (bool, optional): [description]. Defaults to True.

    Returns:
        np.ndarray[, np.ndarray]: return tiles and coords if only_tiles=False
    """

    coords = compute_coords(image,
                            patch_size=tile_size,
                            precompute=True,
                            min_patch_info=min_patch_info,
                            min_axis_info=min_axis_info,
                            min_consec_axis_info=min_consec_axis_info,
                            min_decimal_keep=min_decimal_keep)

    # sort coords (high info -> low info)
    coords = sorted(coords, key=lambda x: x[0], reverse=False)

    tiles = np.empty((len(coords), tile_size, tile_size, 3), dtype=np.uint8)
